 ...
 * @param {DOMEventTarget} topLevelTarget The listening js root node.
...
assign(CallbackQueue.prototype, {
   ...
   * the DOM representation of a js has been created or updated.
   *
   * @internal
   */
  notifyAll: function () {
...
function manualDispatchChangeEvent(nativeEvent) {
  ...
  // the js is rerendered and the value reverted before all the event
...
var DOMChildrenOperations = {
  ...
  /**
   * Updates a js's children by processing a series of updates. The
...
var LinkedStateMixin = {
  /**
   * Create a ReactLink that's linked to part of this js's state. The
...
var LinkedValueUtils = {
  ...
  /**
   * @param {object} inputProps Props for form js
...
var ReactBrowserComponentMixin = {
  /**
   * Returns the DOM node rendered by this js.
   *
   * @return {DOMElement} The root node of this js.
   * @final
   * @protected
   */
  getDOMNode: function () {
...
var ReactClassInterface = {
  ...
  /**
   * An array of Mixin objects to include when defining your js.
   *
   * @type {array}
   * @optional
   */
   ...
   * the js's constructor instead of its prototype (static methods).
   *
   * @type {object}
   * @optional
   */
  ...
  /**
   * Definition of prop types for this js.
   *
   * @type {object}
   * @optional
   */
  ...
  /**
   * Definition of context types for this js.
   *
   * @type {object}
   * @optional
   */
  ...
  /**
   * Definition of context types this js sets for its children.
   *
   * @type {object}
   * @optional
   */
  ...
  /**
   * Invoked when the js is mounted. Values in the mapping will be set on
  ...
  /**
   * Invoked once before the js is mounted. The return value will be used
  ...
  /**
   * Invoked when the js is initially created and about to be mounted.
  ...
  /**
   * Invoked when the js has been mounted and has a DOM representation.
   * However, there is no guarantee that the DOM node is in the document.
   *
   * Use this as an opportunity to operate on the DOM when the js has
   * been mounted (initialized and rendered) for the first time.
   *
   * @param {DOMElement} rootNode DOM element representing the js.
   * @optional
   */
  componentDidMount: SpecPolicy.DEFINE_MANY,

  /**
   * Invoked before the js receives new props.
   *
  ...
  /**
   * Invoked while deciding if the js should be updated as a result of
   * receiving new props, state and/or context.
   *
   * Use this as an opportunity to `return false` when you're certain that the
   * transition to the new props/state/context will not require a js
   * update.
   *
   ...
   * @return {boolean} True if the js should update.
   * @optional
   */
  shouldComponentUpdate: SpecPolicy.DEFINE_ONCE,

  /**
   * Invoked when the js is about to update due to a transition from
  ...
  /**
   * Invoked when the js's DOM representation has been updated.
   *
   * Use this as an opportunity to operate on the DOM when the js has
   * been updated.
   *
  ...
  /**
   * Invoked when the js is about to be removed from its parent and have
   * its DOM representation destroyed.
   *
   * Use this as an opportunity to deallocate any external resources.
   *
   * NOTE: There is no `componentDidUnmount` since your js will have been
  ...
  /**
   * Updates the js's currently mounted DOM representation.
   *
...
function validateMethodOverride(proto, name) {
  ...
  if (proto.hasOwnProperty(name)) {
    !(specPolicy === SpecPolicy.DEFINE_MANY || specPolicy === SpecPolicy.DEFINE_MANY_MERGED) ? "development" !== 'production' ? invariant(false, 'ReactClassInterface: You are attempting to define ' + '`%s` on your js more than once. This conflict may be due ' + 'to a mixin.', name) : invariant(false) : undefined;
  }
}
...
function mixSpecIntoComponent(Constructor, spec) {
  ...
  !(typeof spec !== 'function') ? "development" !== 'production' ? invariant(false, 'ReactClass: You\'re attempting to ' + 'use a js class as a mixin. Instead, just use a regular object.') : invariant(false) : undefined;
  !!ReactElement.isValidElement(spec) ? "development" !== 'production' ? invariant(false, 'ReactClass: You\'re attempting to ' + 'use a js as a mixin. Instead, just use a regular object.') : invariant(false) : undefined;
    ...
    if (RESERVED_SPEC_KEYS.hasOwnProperty(name)) {
      ...
      if (shouldAutoBind) {
        ...
        if (isAlreadyDefined) {
          ...
          !(isReactClassMethod && (specPolicy === SpecPolicy.DEFINE_MANY_MERGED || specPolicy === SpecPolicy.DEFINE_MANY)) ? "development" !== 'production' ? invariant(false, 'ReactClass: Unexpected spec policy %s for key %s ' + 'when mixing in js specs.', specPolicy, name) : invariant(false) : undefined;
...
function mixStaticSpecIntoComponent(Constructor, statics) {
  ...
  for (var name in statics) {
    ...
    !!isInherited ? "development" !== 'production' ? invariant(false, 'ReactClass: You are attempting to define ' + '`%s` on your js more than once. This conflict may be ' + 'due to a mixin.', name) : invariant(false) : undefined;
  ...
  if ("development" !== 'production') {
    ...
    boundMethod.bind = function (newThis) {
      ...
      if (newThis !== component && newThis !== null) {
        "development" !== 'production' ? warning(false, 'bind(): React js methods may only be bound to the ' + 'js instance. See %s', componentName) : undefined;
      } else if (!args.length) {
        "development" !== 'production' ? warning(false, 'bind(): You are binding a js method to the js. ' + 'React does this for you automatically in a high-performance ' + 'way, so you can safely remove this call. See %s', componentName) : undefined;
        return boundMethod;
      }
...
/**
 * Binds all auto-bound methods in a js.
 *
 * @param {object} component Component whose method is going to be bound.
 */
...
var ReactClassMixin = {
  ...
  /**
   * Checks whether or not this composite js is mounted.
...
var ReactClass = {
  ...
  /**
   * Creates a composite js class given a class specification.
   *
  ...
  createClass: function (spec) {
    var Constructor = function (props, context, updater) {
      ...
      if ("development" !== 'production') {
        "development" !== 'production' ? warning(this instanceof Constructor, 'Something is calling a React js directly. Use a factory or ' + 'JSX instead. See: https://fb.me/react-legacyfactory') : undefined;
      }
    ...
    if ("development" !== 'production') {
      "development" !== 'production' ? warning(!Constructor.prototype.componentShouldUpdate, '%s has a method called ' + 'componentShouldUpdate(). Did you mean shouldComponentUpdate()? ' + 'The name is phrased as a question because the function is ' + 'expected to return a value.', spec.displayName || 'A js') : undefined;
      "development" !== 'production' ? warning(!Constructor.prototype.componentWillRecieveProps, '%s has a method called ' + 'componentWillRecieveProps(). Did you mean componentWillReceiveProps()?', spec.displayName || 'A js') : undefined;
    }
...
/**
 * Base class helpers for the updating state of a js.
 */
 ...
 * js arguments (state, props, context). These values can be different
 ...
 * js's state has changed but `setState` was not called.
 *
...
if ("development" !== 'production') {
  var deprecatedAPIs = {
    getDOMNode: ['getDOMNode', 'Use ReactDOM.findDOMNode(js) instead.'],
...
var ReactComponentEnvironment = {
   ...
   * browser etc). Example: A browser system caches DOM nodes based on js
...
/**
 * If your React js's render function is "pure", e.g. it will render the
...
/**
 * An incrementing ID assigned to each js when it is mounted. This is
...
var ReactCompositeComponentMixin = {
  ...
  /**
   * Base constructor for all composite js.
   *
  ...
  /**
   * Initializes the js, renders markup, and registers event listeners.
   *
  ...
  mountComponent: function (rootID, transaction, context) {
    ...
    // js, which is not newable. It might not be 100% reliable but is
    ...
    if ("development" !== 'production') {
      ...
      if (inst.render == null) {
        "development" !== 'production' ? warning(false, '%s(...): No `render` method found on the returned js ' + 'instance: you may have forgotten to define `render`, returned ' + 'null/false from a stateless js, or tried to render an ' + 'element whose type is a function that isn\'t a React js.', Component.displayName || Component.name || 'Component') : undefined;
      } else {
        // We support ES6 inheriting from React.Component, the module pattern,
        // and stateless components, but not ES6 classes that don't extend
        "development" !== 'production' ? warning(Component.prototype && Component.prototype.isReactComponent || !canInstantiate || !(inst instanceof Component), '%s(...): React js classes must extend React.Component.', Component.displayName || Component.name || 'Component') : undefined;
      }
    }
    ...
    if ("development" !== 'production') {
      ...
      "development" !== 'production' ? warning(!inst.getInitialState || inst.getInitialState.isReactClassApproved, 'getInitialState was defined on %s, a plain JavaScript class. ' + 'This is only supported for classes created using React.createClass. ' + 'Did you mean to define a state property instead?', this.getName() || 'a js') : undefined;
      "development" !== 'production' ? warning(!inst.getDefaultProps || inst.getDefaultProps.isReactClassApproved, 'getDefaultProps was defined on %s, a plain JavaScript class. ' + 'This is only supported for classes created using React.createClass. ' + 'Use a static property to define defaultProps instead.', this.getName() || 'a js') : undefined;
      "development" !== 'production' ? warning(!inst.propTypes, 'propTypes was defined as an instance property on %s. Use a static ' + 'property to define propTypes instead.', this.getName() || 'a js') : undefined;
      "development" !== 'production' ? warning(!inst.contextTypes, 'contextTypes was defined as an instance property on %s. Use a ' + 'static property to define contextTypes instead.', this.getName() || 'a js') : undefined;
      "development" !== 'production' ? warning(typeof inst.componentShouldUpdate !== 'function', '%s has a method called ' + 'componentShouldUpdate(). Did you mean shouldComponentUpdate()? ' + 'The name is phrased as a question because the function is ' + 'expected to return a value.', this.getName() || 'A js') : undefined;
      "development" !== 'production' ? warning(typeof inst.componentDidUnmount !== 'function', '%s has a method called ' + 'componentDidUnmount(). But there is no such lifecycle method. ' + 'Did you mean componentWillUnmount()?', this.getName() || 'A js') : undefined;
      "development" !== 'production' ? warning(typeof inst.componentWillRecieveProps !== 'function', '%s has a method called ' + 'componentWillRecieveProps(). Did you mean componentWillReceiveProps()?', this.getName() || 'A js') : undefined;
    }
    ...
    // If not a stateless js, we now render
  ...
  unmountComponent: function () {
    ...
    // Even if this js is scheduled for another update in ReactUpdates,
  ...
  /**
   * Perform an update to a mounted js. The componentWillReceiveProps and
  ...
  updateComponent: function (transaction, prevParentElement, nextParentElement, prevUnmaskedContext, nextUnmaskedContext) {
    ...
    if (prevParentElement === nextParentElement) {
      ...
      // warning for DOM js props in this upgrade
    ...
    if (shouldUpdate) {
      ...
      // If it's determined that a js should not update, we still want
  ...
  /**
   * Call the js's `render` method and update the DOM accordingly.
   *
   * @param {ReactReconcileTransaction} transaction
   * @internal
   */
  ...
  /**
   * Lazily allocates the refs object and stores `js` as `ref`.
   *
  ...
  attachRef: function (ref, component) {
    ...
    if ("development" !== 'production') {
      var componentName = component && component.getName ? component.getName() : 'a js';
  ...
  /**
   * Get a text description of the js that can be used to identify it
   * in error messages.
  ...
  /**
   * Get the publicly accessible representation of this js - i.e. what
   * is exposed by refs and returned by render. Can be null for stateless
   * components.
   *
   * @return {ReactComponent} the public js instance.
   * @internal
   */
 ...
 *
 * The current owner is the js who should own any components that are
...
/**
 * Implements a <button> native js that does not receive mouse events
...
function assertValidProps(component, props) {
  ...
  if ("development" !== 'production') {
    ...
    "development" !== 'production' ? warning(!props.contentEditable || props.children == null, 'A js is `contentEditable` and contains `children` managed by ' + 'React. It is now your responsibility to guarantee that none of ' + 'those nodes are unexpectedly modified or duplicated. This is ' + 'probably not intentional.') : undefined;
  }
...
function trapBubbledEventsLocal() {
  var inst = this;
  // If a js renders to null or if another js fatals and causes
...
ReactDOMComponent.Mixin = {
  ...
  /**
   * Receives a next element and updates the js.
   *
   * @internal
  ...
  /**
   * Updates a native DOM js after it has already been allocated and
...
var ReactDOMIDOperations = {
  ...
  /**
   * Updates a js's children by processing a series of updates.
   *
   * @param {array<object>} updates List of update configurations.
...
function forceUpdateIfMounted() {
  if (this._rootNodeID) {
    // DOM js is still mounted; update
    ReactDOMInput.updateWrapper(this);
  }
}

/**
 * Implements an <input> native js that allows setting these optional
...
/**
 * Implements an <option> native js that warns when `selected` is set.
 */
var ReactDOMOption = {
...
/**
 * Implements a <select> native js that allows optionally setting the
 ...
 *
 * This js is used to wrap strings in elements so that they can undergo
...
assign(ReactDOMTextComponent.prototype, {
  ...
  /**
   * Updates this js by updating the text content.
   *
...
/**
 * Implements a <textarea> native js that allows setting `value`, and
...
function getTotalTime(measurements) {
  ...
  // TODO: log total time of each reconcile and the top-level js
...
function getUnchangedComponents(measurement) {
  ...
  for (var id in allIDs) {
    var isDirty = false;
    // For each js that rendered, see if a js that triggered
...
var ReactElement = function (type, key, ref, self, source, owner, props) {
  var element = {
    ...
    // Record the js responsible for creating this element.
    _owner: owner
  };
...
/**
 * @param {?object} object
 * @return {boolean} True if `object` is a valid js.
 * @final
 */
...
function getAddendaForKeyUse(messageType, element, parentType) {
  ...
  if (element && element._owner && element._owner !== ReactCurrentOwner.current) {
    // Give the js that originally created this child.
 ...
 * Assert that the props are valid
 *
 * @param {string} componentName Name of the js for error messages.
 * @param {object} propTypes Map of prop name to a ReactPropType
 * @param {object} props
 * @param {string} location e.g. "prop", "context", "child context"
 * @private
 ...
 * @return {boolean} True if the js is rendered to null.
 */
...
/**
 * Mark the js as having rendered to null.
 * @param {string} id Component's `_rootNodeID`.
 */
...
/**
 * Unmark the js as having rendered to null: it renders to something now.
 * @param {string} id Component's `_rootNodeID`.
 */
...
var ReactEventEmitterMixin = {
   ...
   *
   * @param {string} topLevelType Record from `EventConstants`.
   * @param {object} topLevelTarget The listening js root node.
   * @param {string} topLevelTargetID ID of `topLevelTarget`.
...
/**
 * Finds the parent React js of `node`.
 *
 * @param {*} node
...
var ReactInstanceHandles = {
   ...
   *
   * @param {string} rootID Root ID of a parent js.
   * @param {string} name A js's name (as flattened children).
  ...
  /**
   * Gets the DOM ID of the React js that is the root of the tree that
   * contains the React js with the supplied DOM ID.
   *
   * @param {string} id DOM ID of a React js.
   * @return {?string} DOM ID of the React js that is the root.
   * @internal
   */
...
/**
 * ReactLink encapsulates a common pattern in which a js wants to modify
...
/** Mapping from reactRootID to React js instance. */
...
/**
 * @param {DOMElement} container DOM element that may contain a React js.
 * @return {?string} A "reactRoot" ID, if a React js is rendered.
 */
...
/**
 * Mounts this js and inserts it into the DOM.
 *
...
/**
 * Unmounts a js and removes it from the DOM.
 *
 * @param {ReactComponent} instance React js instance.
...
function findFirstReactDOMImpl(node) {
  ...
  for (; node && node.parentNode !== node; node = node.parentNode) {
    if (node.nodeType !== 1) {
      // Not a DOMElement, therefore not a React js
      continue;
    }
...
/**
 * Mounting is the process of initializing a React js by creating its
...
var ReactMount = {
  ...
  /**
   * Take a js that's already mounted into the DOM and replace its props
   * @param {ReactComponent} prevComponent js instance already in the DOM
   * @param {ReactElement} nextElement js instance to render
  ...
  /**
   * Register a js into the instance map and starts scroll value
   * monitoring
   * @param {ReactComponent} nextComponent js instance to render
   * @param {DOMElement} container container to render into
  ...
  _renderNewRootComponent: function (nextElement, container, shouldReuseMarkup, context) {
    ...
    "development" !== 'production' ? warning(ReactCurrentOwner.current == null, '_renderNewRootComponent(): Render methods should be a pure function ' + 'of props and state; triggering nested js updates from ' + 'render is not allowed. If necessary, trigger nested updates in ' + 'componentDidUpdate. Check the render method of %s.', ReactCurrentOwner.current && ReactCurrentOwner.current.getName() || 'ReactCompositeComponent') : undefined;
  ...
  /**
   * Renders a React js into the DOM in the supplied `container`.
   *
   * If the React js was previously rendered into `container`, this will
  ...
  _renderSubtreeIntoContainer: function (parentComponent, nextElement, container, callback) {
    !ReactElement.isValidElement(nextElement) ? "development" !== 'production' ? invariant(false, 'ReactDOM.render(): Invalid js element.%s', typeof nextElement === 'string' ? ' Instead of passing an element string, make sure to instantiate ' + 'it by passing it to React.createElement.' : typeof nextElement === 'function' ? ' Instead of passing a js class, make sure to instantiate ' + 'it by passing it to React.createElement.' :
  ...
  /**
   * Unmounts and destroys the React js rendered in the `container`.
   *
   * @param {DOMElement} container DOM element containing a React js.
   * @return {boolean} True if a js was found in and unmounted from
   *                   `container`
   */
  unmountComponentAtNode: function (container) {
    ...
    if (!component) {
      ...
      if ("development" !== 'production') {
        "development" !== 'production' ? warning(!containerHasNonRootReactChild, 'unmountComponentAtNode(): The node you\'re attempting to unmount ' + 'was rendered by React and is not a top-level container. %s', isContainerReactRoot ? 'You may have accidentally passed in a React root node instead ' + 'of its container.' : 'Instead, have the parent js update its state and ' + 'rerender in order to remove this js.') : undefined;
      }
  ...
  /**
   * Finds the container DOM element that contains React js to which the
   * supplied DOM `id` belongs.
   *
   * @param {string} id The ID of an element rendered by a React js.
   ...
   *
   * @param {string} id ID of a DOM node in the React js.
   * @return {DOMElement} Root DOM node of the React js.
   */
   ...
   * DOM representation of a React js rendered by this copy of React.
   *
   * @param {*} node
   ...
   * @pararm {string} targetID ID of the DOM representation of the js.
  ...
  findComponentRoot: function (ancestorNode, targetID) {
    ...
    if ("development" !== 'production') {
      ...
      "development" !== 'production' ? warning(deepestAncestor != null, 'React can\'t find the root js node for data-reactid value ' + '`%s`. If you\'re seeing this message, it probably means that ' + 'you\'ve loaded two copies of React on the page. At this time, only ' + 'a single copy of React can be loaded at a time.', targetID) : undefined;
    }
  ...
  _mountImageIntoNode: function (markup, container, shouldReuseMarkup, transaction) {
    ...
    if (shouldReuseMarkup) {
      ...
      if (ReactMarkupChecksum.canReuseMarkup(markup, rootElement)) {
        ...
        !(container.nodeType !== DOC_NODE_TYPE) ? "development" !== 'production' ? invariant(false, 'You\'re trying to render a js to the document using ' + 'server rendering but the checksum was invalid. This usually ' + 'means you rendered a different js type or props on ' + 'the client from the one on the server, or your render() ' + 'methods are impure. React cannot handle this case due to ' + 'cross-browser quirks by rendering at the document root. You ' + 'should look for environment dependent code in your components ' + 'and ensure the props are the same client and server side:\n%s', difference) : invariant(false) : undefined;
    ...
    !(container.nodeType !== DOC_NODE_TYPE) ? "development" !== 'production' ? invariant(false, 'You\'re trying to render a js to the document but ' + 'you didn\'t use server rendering. We can\'t do this ' + 'without using server rendering due to cross-browser quirks. ' + 'See ReactDOMServer.renderToString() for server rendering.') : invariant(false) : undefined;
...
/**
 * Updating children of a js may trigger recursive updates. The depth is
 ...
 *
 * @param {string} parentID ID of the parent js.
...
var ReactMultiChild = {
  ...
  Mixin: {
    ...
    /**
     * Moves a child js to the supplied index.
     *
...
/**
 * When a js's children are updated, a series of update configuration
...
var ReactNativeComponentInjection = {
  ...
  // This accepts a text js class that takes the text string to be
  // rendered as props.
...
/**
 * Get a composite js wrapper class for a specific tag.
 *
...
/**
 * Get a native internal js class for a specific tag.
 *
 * @param {ReactElement} element The element to create.
 * @return {function} The internal class constructor function.
 */
function createInternalComponent(element) {
  !genericComponentClass ? "development" !== 'production' ? invariant(false, 'There is no registered js for the tag %s', element.type) : invariant(false) : undefined;
...
function warnTDZ(publicInstance, callerName) {
  if ("development" !== 'production') {
    "development" !== 'production' ? warning(false, '%s(...): Can only update a mounted or mounting js. ' + 'This usually means you called %s() on an unmounted js. ' + 'This is a no-op. Please check the code for the %s js.', callerName, callerName, publicInstance.constructor && publicInstance.constructor.displayName || '') : undefined;
  }
}
...
var ReactOwner = {
  ...
  /**
   * Adds a js by ref to an owner js.
   *
   * @param {ReactComponent} component Component to reference.
   * @param {string} ref Name by which to refer to the js.
   * @param {ReactOwner} owner Component on which to record the ref.
   * @final
   * @internal
   */
  addComponentAsRefTo: function (component, ref, owner) {
    !ReactOwner.isValidOwner(owner) ? "development" !== 'production' ? invariant(false, 'addComponentAsRefTo(...): Only a ReactOwner can have refs. You might ' + 'be adding a ref to a js that was not created inside a js\'s ' + '`render` method, or you have multiple copies of React loaded ' + '(details: https://fb.me/react-refs-must-have-owner).') : invariant(false) : undefined;
    owner.attachRef(ref, component);
  },

  /**
   * Removes a js by ref from an owner js.
   *
  ...
  removeComponentAsRefFrom: function (component, ref, owner) {
    !ReactOwner.isValidOwner(owner) ? "development" !== 'production' ? invariant(false, 'removeComponentAsRefFrom(...): Only a ReactOwner can have refs. You might ' + 'be removing a ref to a js that was not created inside a js\'s ' + '`render` method, or you have multiple copies of React loaded ' + '(details: https://fb.me/react-refs-must-have-owner).') : invariant(false) : undefined;
    // Check that `js` is still the current ref because we do not want to
    // detach the ref if another js stole it.
...
/**
 * ReactPropTransferer are capable of transferring props to another js
...
/**
 * Helper to call ReactRef.attachRefs with this composite js, split out
...
var ReactReconciler = {
  ...
  /**
   * Update a js using a new element.
   *
   * @param {ReactComponent} internalInstance
   * @param {ReactElement} nextElement
   * @param {ReactReconcileTransaction} transaction
  ...
  /**
   * Flush any dirty changes in a js.
   *
   * @param {ReactComponent} internalInstance
   * @param {ReactReconcileTransaction} transaction
   * @internal
...
var ReactTestUtils = {
  ...
  findAllInRenderedTree: function (inst, test) {
    ...
    !ReactTestUtils.isCompositeComponent(inst) ? "development" !== 'production' ? invariant(false, 'findAllInRenderedTree(...): instance must be a composite js') : invariant(false) : undefined;
  ...
  /**
   * Pass a mocked js module to this method to augment it with
   * useful methods that allow it to be used as a dummy React js.
   * Instead of rendering as usual, the js will become a simple
   * <div> containing any provided children.
   *
   * @param {object} module the mock function object exported from a
   *                        module that defines the js to be mocked
...
ReactShallowRenderer.prototype.render = function (element, context) {
  !ReactElement.isValidElement(element) ? "development" !== 'production' ? invariant(false, 'ReactShallowRenderer render(): Invalid js element.%s', typeof element === 'function' ? ' Instead of passing a js class, make sure to instantiate ' + 'it by passing it to React.createElement.' : '') : invariant(false) : undefined;
...
var ReactTransitionGroup = React.createClass({
  ...
  performLeave: function (key) {
    ...
    if (component.componentWillLeave) {
      ...
      // again, effectively mutating the js before all the work
      // is done.
...
function getInternalInstanceReadyForUpdate(publicInstance, callerName) {
  ...
  if (!internalInstance) {
    if ("development" !== 'production') {
      ...
      "development" !== 'production' ? warning(!callerName, '%s(...): Can only update a mounted or mounting js. ' + 'This usually means you called %s() on an unmounted js. ' + 'This is a no-op. Please check the code for the %s js.', callerName, callerName, publicInstance.constructor.displayName) : undefined;
    }
    return null;
  }
...
var ReactUpdateQueue = {
  ...
  isMounted: function (publicInstance) {
    if ("development" !== 'production') {
      ...
      if (owner !== null) {
        "development" !== 'production' ? warning(owner._warnedAboutRefsInRender, '%s is accessing isMounted inside its render() function. ' + 'render() should be a pure function of props and state. It should ' + 'never access something that requires stale data from the previous ' + 'render, such as refs. Move this logic to componentDidMount and ' + 'componentDidUpdate instead.', owner.getName() || 'A js') : undefined;
  ...
  enqueueSetPropsInternal: function (internalInstance, partialProps) {
    ...
    !topLevelWrapper ? "development" !== 'production' ? invariant(false, 'setProps(...): You called `setProps` on a ' + 'js with a parent. This is an anti-pattern since props will ' + 'get reactively updated when rendered. Instead, change the owner\'s ' + '`render` method to pass the correct value as props to the js ' + 'where it is created.') : invariant(false) : undefined;
  ...
  enqueueReplacePropsInternal: function (internalInstance, props) {
    var topLevelWrapper = internalInstance._topLevelWrapper;
    !topLevelWrapper ? "development" !== 'production' ? invariant(false, 'replaceProps(...): You called `replaceProps` on a ' + 'js with a parent. This is an anti-pattern since props will ' + 'get reactively updated when rendered. Instead, change the owner\'s ' + '`render` method to pass the correct value as props to the js ' + 'where it is created.') : invariant(false) : undefined;
 ...
 *
 * @param {ReactComponent} c1 first js you're comparing
 * @param {ReactComponent} c2 second js you're comparing
...
function runBatchedUpdates(transaction) {
  ...
  // Since reconciling a js higher in the owner hierarchy usually (not
  // always -- see shouldComponentUpdate()) will reconcile children, reconcile
  // them before their children by sorting the array.
  dirtyComponents.sort(mountOrderComparator);

  for (var i = 0; i < len; i++) {
    // If a js is unmounted before pending changes apply, it will still
...
/**
 * Mark a js as needing a rerender, adding an optional callback to a
...
function findDOMNode(componentOrElement) {
  ...
  !(componentOrElement.render == null || typeof componentOrElement.render !== 'function') ? "development" !== 'production' ? invariant(false, 'findDOMNode was called on an unmounted js.') : invariant(false) : undefined;
 ...
 * @param {?ReactComponent} child React child js.
...
/**
 * Generate a key string that identifies a js within a set.
 *
 * @param {*} component A js that could contain a manual key.
...
/**
 * Escape a js key so that it is safe to use in a reactid.
 *
 ...
 * implicitly-generated keys generated by a js's index in its parent.
 *
