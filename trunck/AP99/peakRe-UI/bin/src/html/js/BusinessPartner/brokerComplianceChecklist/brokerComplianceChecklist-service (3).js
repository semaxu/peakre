(function (context) {
    ...
    service.save = function (model, quiet, async, done, fail) {
        var url = $ri.getRestfulURL("action.public.partner") + "/createBrokerCompliance";
        $pt.doPost(url, model, {
            quiet: quiet,
            async: async,
            done: done,
    ...
    service.load = function (model, quiet, async, done, fail) {
        var url = $ri.getRestfulURL("action.public.partner") + "/loadBrokerCompliance";
        $pt.doGet(url, model, {
            quiet: quiet,
            async: async,
            done: done,
    ...
    service.print = function (model, quiet, async, done, fail) {
        var url = $ri.getRestfulURL("action.public.file") + "/printURL/17";
        $pt.doPost(url, model, {
            quiet: quiet,
            async: async,
            done: done,
