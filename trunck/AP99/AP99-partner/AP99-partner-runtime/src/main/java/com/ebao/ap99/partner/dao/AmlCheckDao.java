...
public class AmlCheckDao extends BaseDao<TAmlCheck> {
	
	@PersistenceContext
	private EntityManager em;
	
	@Override
	public Class<TAmlCheck> getEntityClass() {
		return TAmlCheck.class;
	}
    
    public List<TAmlCheck> findListByPartner(TPartner partner) {
        StringBuffer sqlBuf = new StringBuffer("SELECT t FROM TAmlCheck t WHERE 1=1");
        ...
        List<TAmlCheck> list = (List<TAmlCheck>) query.getResultList();
        return list;
    }
    ...
    public boolean  findAmlCheck(TPartner partner){
        ...
        sql.append(" SELECT t.* FROM T_RI_BP_AML_CHECK t where  add_months(t.check_date ,12) > sysdate  ");
    ...
    public boolean  findDeclinedAmlCheck(Partner partner){
        boolean flag = false;
        StringBuilder sql = new StringBuilder();
        sql.append(" SELECT t.aml_check_result FROM T_RI_BP_AML_CHECK t where  1 = 1  ");
