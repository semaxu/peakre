...
public class BrokerAmlCheckDao extends BaseDao<TBrokerAmlCheck> {
	
	@PersistenceContext
	private EntityManager em;
	
	@Override
	public Class<TBrokerAmlCheck> getEntityClass() {
		return TBrokerAmlCheck.class;
	}
    
    public List<TBrokerAmlCheck> findListByPartner(TPartner partner) {
        StringBuffer sqlBuf = new StringBuffer("SELECT t FROM TBrokerAmlCheck t WHERE 1=1");
        ...
        List<TBrokerAmlCheck> list = (List<TBrokerAmlCheck>) query.getResultList();
        return list;
    }
    ...
    public boolean  findAmlCheck(TPartner partner){
        ...
        sql.append(" SELECT t.* FROM T_RI_BP_BROKER_AML_CHECK t where  add_months(t.check_date ,12) > sysdate  ");
        
        if (partner.getPartnerId()!=0L) {
            sql.append(" and t.partner_id='").append(partner.getPartnerId()).append("'");         
        }

        List<TBrokerAmlCheck> amlCheckList = this.getJdbcTemplate().query(sql.toString(), new Object[] {},
                new BeanPropertyRowMapper<TBrokerAmlCheck>(TBrokerAmlCheck.class));
    ...
    public boolean  findDeclinedAmlCheck(Partner partner){
        boolean flag = false;
        StringBuilder sql = new StringBuilder();
        sql.append(" SELECT t.aml_check_result FROM T_RI_BP_BROKER_AML_CHECK t where  1 = 1  ");
